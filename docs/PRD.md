...
The kit is meant to be **forked**. Drop this whole `editor-starter/` directory into a new repo, swap the six personalization slots, and ship your own version. The blank PRD, brand-guide, and implementation templates ship in **[builder-os](https://github.com/v60samurai/builder-os)** — the companion repo.
...
**Stack**: Lucia + Arctic + Oslo for session-based auth (matches the rest of the stack guidance in the Implementation Guide template — [builder-os](https://github.com/v60samurai/builder-os)). PostgreSQL holds `users` and `sessions` tables. Magic links are sent via Resend (or any SMTP — adapter in `src/lib/email.ts`).
...
- The build sequence from the Session Playbook (`SESSION_PLAYBOOK.md` in [builder-os](https://github.com/v60samurai/builder-os)), adapted for Editor — auth lands in Session 3, memory in Session 4 alongside the core pipeline.
...
*Editor Starter PRD v2, Harshit Badiger, 2026-05-17. The brand sheet is at `docs/BRAND.md`. The Implementation Guide template (`IMPLEMENTATION_GUIDE.md` in [builder-os](https://github.com/v60samurai/builder-os)) is what you fill in for your fork.*
